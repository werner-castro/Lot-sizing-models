# Capacitated lot-sizing problem (CLSP) / Modelo de dimensionamento de lotes multi-ítem com restrição de capacidade e tempo de setup
# 
# modelo retirado do livro: Deterministic Lotsizing Models for Production Planning, Marc Salomon, pg = 33
...
#  J
#  ∑ (b(j) * x(j,t) + a(j) * y(j,t)) ≤ C(t)            ∀ t = 1..T  # restrição de capacidade com a adição do tempo de setup
# j=1
...
# Parâmetros
a = rand(1:4,J,1)               # setup time / tempo de setup
...
@constraint(CLSP, [t in 1:T], sum(b[j] * x[j,t] + a[j] * y[j,t] for j = 1:J) ≤ C[t]) 
